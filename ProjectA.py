...
def main_menu(screen, startScreen):
    ...
    #Set up the choices
    #choices = [board.girl, board.boy, board.lion, board.ghost, board.duck, board.shark]
    choices = [board.shark]
...
def main_loop(screen, board, settings):
    background = pygame.Surface(screen.get_size())
    background.fill(white)
    background = pygame.image.load("o.jpg").convert()
    screen.blit(background, (0, 0))
...
##        self.ghost.image = pygame.image.load("halloweenGhost.jpg").convert()
        ...
        self.shark.image = pygame.image.load("shark_right.png").convert()

        self.Choices.add(self.shark)    
        #self.Choices.add(self.girl, self.boy, self.lion, self.ghost, self.duck, self.shark)
